...
const download = () => {
    ...
    const addEvent = () => {
        ...
        const dates = jigenData.map(value => {
            const dt = value.replace(/[^0-9]+/g, '/').split('/').map(str => parseInt(str, 10));
        ...
        for (let i = 0; i < classes.length; i++) {
            ...
            if (!classList.contains('kyuko-kaiko') &&
            ...
            const time = classDetail[1].textContent.replace(/[^0-9,]+/g, '').split(',').map(val => parseInt(val, 10) - 1);
            const jigen = [
                [8.75, 9.5], [9.5, 10.25],
                [10.5, 11.25], [11.25, 12],
                [12.8333333333, 13.5833333333], [13.5833333333, 14.3333333333],
                [14.5833333333, 15.3333333333], [15.3333333333, 16.0833333333],
                [16.3333333333, 17.0833333333], [17.0333333333, 17.8333333333]
            ];
            cal.addEvent(evinfo[4], `${evinfo[0]} - ${evinfo[2]}`, evinfo[1], new Date(dates[i % 7] + 3600000 * jigen[time[0]][0]).toISOString(), new Date(dates[i % 7] + 3600000 * jigen[time.pop()][1]).toISOString());
        }
